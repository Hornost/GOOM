...
func update_material(item: GMapGridItem):#, light_level = -1):
	...
	for i in range(meshes.size()):
		...
		if mesh is MeshInstance3D:
			mesh.mesh = GMapStyles.meshes[mesh.name.split("[")[1].replace("]","")][item.style][clamp(GMapGridItem.MAX_LIGHT_LEVEL-1-item.light_level+(1.0-MESHES_LIGHT_LEVEL[i])*GMapGridItem.MAX_LIGHT_LEVEL,0,GMapGridItem.MAX_LIGHT_LEVEL)]
			#if light_level == -1:
